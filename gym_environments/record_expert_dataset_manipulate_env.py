...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="shadow_hand_block-v0", help="environment ID")
    ...
    # Make environment
    env = gym.make(args.env, render=True, sim_n_sub_steps=args.sub_steps, orientation_threshold=args.threshold)
    ...
    if args.render:
        ...
        imageio.mimsave("ddpg_shadow_hand_block_pretraining.gif", [np.array(img) for n, img in enumerate(images) if n % 2 == 0], fps=10)
        env.close()
    else:
        actions = []
        observations = []
