...
from utils.capture_output import Capturing
...
from utils.callback_success import EvalSuccessCallback
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', help='Pretrain with expert dataset', default=-1, type=int)
    ...
    if args.algo == 'her':
        ...
        assert algo_ in {'sac', 'ddpg', 'dqn', 'td3', 'ddpgfed', 'ddpgfedloss'}, "{} is not compatible with HER".format(algo_)
    ...
    if args.pretrain > -1:
        save_path = os.path.join(log_path,
                                 "{}_{}{}_{}_bc_{}".format(env_id, get_latest_run_id(log_path, env_id) + 1, uuid_str,
                                                           now, args.pretrain))
    else:
        save_path = os.path.join(log_path,
                                 "{}_{}{}_{}".format(env_id, get_latest_run_id(log_path, env_id) + 1, uuid_str, now))
    ...
    def create_env(n_envs, eval_env=False):
        ...
        if algo_ in ['dqn', 'ddpg', 'ddpgfed', 'ddpgfedloss']:
            if hyperparams.get('normalize', False):
                print("WARNING: normalization not supported yet for DDPG/DQN")

            # Never render eval env
            if eval_env and "render" in env_kwargs.keys():
            ...
            if n_envs == 1:
                ...
                env = SubprocVecEnv(
                    [make_env(env_id, i, args.seed, wrapper_class=env_wrapper, env_kwargs=env_kwargs) for i in
                     range(n_envs)])
            if normalize:
                if args.verbose > 0:
    ...
    if args.eval_freq > 0:
        ...
        eval_callback = EvalSuccessCallback(eval_env, callback_on_new_best=save_vec_normalize,
                                            best_model_save_path=save_path, n_eval_episodes=args.eval_episodes,
                                            log_path=save_path, eval_freq=args.eval_freq, rank=rank)
    ...
    if algo_ in ['ddpg', 'sac', 'td3', 'ddpgfed', 'ddpgfedloss'] and hyperparams.get('noise_type') is not None:
        noise_type = hyperparams['noise_type'].strip()
        noise_std = hyperparams['noise_std']
        n_actions = env.action_space.shape[0]
        if 'adaptive-param' in noise_type:
            assert algo_ in ['ddpg', 'ddpgfed', 'ddpgfedloss'], 'Parameter is not supported by SAC'
    ...
    if os.path.isfile(args.trained_agent):
        ...
        if rank == 0:
            model.save("{}/{}_blank".format(save_path, env_id))
    ...
    if args.pretrain > -1:
        expert_path = os.path.join("experts", args.env, "{}_expert.npz".format(args.env))

        print("Pretrain with expert dataset")
        ...
        elif "nminibatches" in hyperparams.keys():
            batch_size = hyperparams["nminibatches"]

        if batch_size > 0:
            data_set = ExpertDataset(expert_path=expert_path, traj_limitation=-1, verbose=1, batch_size=batch_size)
        else:
            data_set = ExpertDataset(expert_path=expert_path, traj_limitation=-1, verbose=1)

        # Pretrain the model
        if args.algo == 'her':
            with Capturing() as output:
                model.model.pretrain(data_set, n_epochs=args.pretrain, val_interval=1)
        else:
            with Capturing() as output:
                model.pretrain(data_set, n_epochs=args.pretrain, val_interval=1)

        results = list(filter(lambda x: "Training loss" in x, output))
        training_loss = []
        validation_loss = []
        ...
        for n in range(len(results)):
            res = results[n]
            res = res.split(",")
            training_loss.append(float(res[0].split(": ")[1]))
            validation_loss.append(float(res[1].split(": ")[1]))
        ...
        np.savez("{}/{}".format(save_path, "pretraining"), **numpy_dict)

        if rank == 0:
            model.save("{}/{}_pretrained".format(save_path, env_id))
