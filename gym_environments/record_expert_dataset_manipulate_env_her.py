...
import pybullet
from gym_shadow_hand.envs.manipulate_env import MOVABLE_JOINTS
...
def normalize_actions(action):
    ...
    for n in range(env_basis.n_model_joints):
        joint_info = pybullet.getJointInfo(env_basis.model_id, n)
...
def get_palm_position():
    ...
    for n in range(env_basis.n_model_joints):
        ...
        if joint_info[12] in [b"rh_palm"]:
            link_state = pybullet.getLinkState(env_basis.model_id, n)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="shadow_hand_block_goalenv-v0", help="environment ID")
    ...
    parser.add_argument("--threshold", help="threshold for episode end", default=0.1, type=float)
    ...
    # Make environment
    env_basis = gym.make(args.env, render=True)
    env = HERGoalEnvWrapper(env_basis)
    ...
    if args.render:
        ...
        imageio.mimsave("her_shadow_hand_manipulate_pretraining.gif",
        ...
        for n in range(args.episodes):
            while True:
                ...
                # done or
                if done or info["is_success"] and observation[-13] <= palm_pos[1] + 0.04:
                    observation = env.reset()
