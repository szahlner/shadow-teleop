...
class TestRightHandThumbPositionAccuracyLimits(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        with plt.style.context('ggplot'):
            ...
            fig.suptitle("Right Hand Thumb DH Bone End-Position Limits, samples: {}".format(len(cls.right_hand_thumb_proximal_dh_position_accuracy)), fontsize=14)

            fig.savefig(os.path.join(test_utils.TEST_DATA_RESULTS_DIRECTORY, "right_hand_thumb_dh_position_accuracy_limits.png"), format="png")
            plt.close()

            test_utils.right_hand_thumb_proximal_dh_position_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_proximal_dh_position_accuracy))
            test_utils.right_hand_thumb_intermediate_dh_position_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_intermediate_dh_position_accuracy))
            test_utils.right_hand_thumb_distal_dh_position_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_distal_dh_position_accuracy))
    ...
    @parameterized.expand([
        ["right_hand_thumb_proximal_dh_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
    ...
    @parameterized.expand([
        ["right_hand_thumb_intermediate_dh_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
    ...
    @parameterized.expand([
        ["right_hand_thumb_distal_dh_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
