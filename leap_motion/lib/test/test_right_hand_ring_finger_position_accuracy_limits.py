...
class TestRightHandIndexFingerPositionAccuracyLimits(unittest.TestCase):
    right_hand_ring_finger_proximal_position_accuracy = []
    right_hand_ring_finger_intermediate_position_accuracy = []
    right_hand_ring_finger_distal_position_accuracy = []

    right_hand_ring_finger_proximal_dh_position_accuracy = []
    right_hand_ring_finger_intermediate_dh_position_accuracy = []
    right_hand_ring_finger_distal_dh_position_accuracy = []
    ...
    def tearDownClass(cls):
        ...
        with plt.style.context('ggplot'):
            ...
            axes[0].hist(cls.right_hand_ring_finger_proximal_position_accuracy, bins=50, facecolor=face_color)
            axes[0].ticklabel_format(axis="x", style="sci", scilimits=(0, 0))
            axes[0].set_ylabel("Quantity")
            axes[0].set_title("Proximal", fontsize=12)

            axes[1].hist(cls.right_hand_ring_finger_intermediate_position_accuracy, bins=50, facecolor=face_color)
            axes[1].ticklabel_format(axis="x", style="sci", scilimits=(0, 0))
            axes[1].set_xlabel("Distance error in mm", labelpad=20)
            axes[1].set_title("Intermediate", fontsize=12)

            axes[2].hist(cls.right_hand_ring_finger_distal_position_accuracy, bins=50, facecolor=face_color)
            axes[2].ticklabel_format(axis="x", style="sci", scilimits=(0, 0))
            axes[2].set_title("Distal", fontsize=12)

            fig.suptitle("Right Hand Ring Finger Bone End-Position Limits, samples: {}".format(len(cls.right_hand_ring_finger_proximal_position_accuracy)), fontsize=14)

            fig.savefig(os.path.join(test_utils.TEST_DATA_RESULTS_DIRECTORY, "right_hand_ring_finger_position_accuracy_limits.png"), format="png")
            plt.close()
            ...
            axes[0].hist(cls.right_hand_ring_finger_proximal_dh_position_accuracy, bins=50, facecolor=face_color)
            axes[0].ticklabel_format(axis="x", style="sci", scilimits=(0, 0))
            axes[0].set_ylabel("Quantity")
            axes[0].set_title("Proximal", fontsize=12)

            axes[1].hist(cls.right_hand_ring_finger_intermediate_dh_position_accuracy, bins=50, facecolor=face_color)
            axes[1].ticklabel_format(axis="x", style="sci", scilimits=(0, 0))
            axes[1].set_xlabel("Distance error in mm", labelpad=20)
            axes[1].set_title("Intermediate", fontsize=12)

            axes[2].hist(cls.right_hand_ring_finger_distal_dh_position_accuracy, bins=50, facecolor=face_color)
            axes[2].ticklabel_format(axis="x", style="sci", scilimits=(0, 0))
            axes[2].set_title("Distal", fontsize=12)

            fig.suptitle("Right Hand Ring Finger DH Bone End-Position Limits, samples: {}".format(len(cls.right_hand_ring_finger_proximal_dh_position_accuracy)), fontsize=14)

            fig.savefig(os.path.join(test_utils.TEST_DATA_RESULTS_DIRECTORY, "right_hand_ring_finger_dh_position_accuracy_limits.png"), format="png")
            plt.close()
    ...
    @parameterized.expand([
        ["right_hand_ring_finger_proximal_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
    ...
    def test_sequence(self, name, num, accuracy):
        ...
        # Add positions
        cur_pos += -new_dir * self.hand[num]["ring_finger"]["proximal"]["length"]

        # Compare
        diff = cur_pos.distance_to(real_pos)
        self.right_hand_ring_finger_proximal_position_accuracy.append(diff)

        self.assertAlmostEqual(diff, 0., places=accuracy)

    @parameterized.expand([
        ["right_hand_ring_finger_proximal_dh_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
        ...
        # Compare
        diff = cur_pos.distance_to(real_pos)
        self.right_hand_ring_finger_proximal_dh_position_accuracy.append(diff)

        self.assertAlmostEqual(diff, 0., places=accuracy)

    @parameterized.expand([
        ["right_hand_ring_finger_intermediate_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
        ...
        # Add positions
        cur_pos += -new_dir * self.hand[num]["ring_finger"]["intermediate"]["length"]

        # Compare
        diff = cur_pos.distance_to(real_pos)
        self.right_hand_ring_finger_intermediate_position_accuracy.append(diff)

        self.assertAlmostEqual(diff, 0., places=accuracy)

    @parameterized.expand([
        ["right_hand_ring_finger_intermediate_dh_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
        ...
        # Compare
        diff = cur_pos.distance_to(real_pos)
        self.right_hand_ring_finger_intermediate_dh_position_accuracy.append(diff)

        self.assertAlmostEqual(diff, 0., places=accuracy)

    @parameterized.expand([
        ["right_hand_ring_finger_distal_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
        ...
        # Add positions
        cur_pos += -new_dir * self.hand[num]["ring_finger"]["distal"]["length"]

        # Compare
        diff = cur_pos.distance_to(real_pos)
        self.right_hand_ring_finger_distal_position_accuracy.append(diff)

        self.assertAlmostEqual(diff, 0., places=accuracy)

    @parameterized.expand([
        ["right_hand_ring_finger_distal_dh_position_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
        ...
        # Compare
        diff = cur_pos.distance_to(real_pos)
        self.right_hand_ring_finger_distal_dh_position_accuracy.append(diff)
