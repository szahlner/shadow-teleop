...
class TestRightHandThumbDirectionAccuracyLimits(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        with plt.style.context('ggplot'):
            ...
            fig.suptitle("Right Hand Thumb DH Bone Direction Limits, samples: {}".format(len(cls.right_hand_thumb_proximal_dh_direction_x_accuracy)), fontsize=14)

            fig.savefig(os.path.join(test_utils.TEST_DATA_RESULTS_DIRECTORY, "right_hand_thumb_dh_direction_accuracy_limits.png"), format="png")
            plt.close()

            test_utils.right_hand_thumb_proximal_dh_direction_x_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_proximal_dh_direction_x_accuracy))
            test_utils.right_hand_thumb_proximal_dh_direction_y_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_proximal_dh_direction_y_accuracy))
            test_utils.right_hand_thumb_proximal_dh_direction_z_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_proximal_dh_direction_z_accuracy))

            test_utils.right_hand_thumb_intermediate_dh_direction_x_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_intermediate_dh_direction_x_accuracy))
            test_utils.right_hand_thumb_intermediate_dh_direction_y_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_intermediate_dh_direction_y_accuracy))
            test_utils.right_hand_thumb_intermediate_dh_direction_z_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_intermediate_dh_direction_z_accuracy))

            test_utils.right_hand_thumb_distal_dh_direction_x_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_distal_dh_direction_x_accuracy))
            test_utils.right_hand_thumb_distal_dh_direction_y_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_distal_dh_direction_y_accuracy))
            test_utils.right_hand_thumb_distal_dh_direction_z_accuracy_limits_mean = np.abs(np.mean(cls.right_hand_thumb_distal_dh_direction_z_accuracy))
    ...
    @parameterized.expand([
        ["right_hand_thumb_proximal_dh_direction_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
    ...
    def test_sequence(self, name, num, accuracy):
        ...
        theta1 = np.clip(-np.arctan2(vec.x, vec.y) + np.pi, -1.047, 1.047)
        theta2 = np.clip(np.pi - np.arccos(vec.z / l2), 0., 1.222)
    ...
    @parameterized.expand([
        ["right_hand_thumb_intermediate_dh_direction_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
        ...
        # From inverse kinematics (DH-convention)
        theta3 = np.clip(np.arctan(vec.y / vec.z), -0.209, 0.209)
        theta4 = np.clip(np.arcsin(vec.x / l4), -0.524, 0.524)
    ...
    @parameterized.expand([
        ["right_hand_thumb_distal_dh_direction_accuracy_limits_{}".format(x), x, wrapper.args["accuracy"]]
        for x in range(test_utils.TEST_NUMBER_DATA_SAMPLES)],
        skip_on_empty=True)
    def test_sequence(self, name, num, accuracy):
        """
        ...
        # From inverse kinematics (DH-convention)
        theta5 = np.clip(np.pi - np.arccos(vec.z / l5), 0., 1.571)
