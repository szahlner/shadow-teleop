...
class LeapMotionListenerFloatingHand(Leap.Listener):
    ...
    def __init__(self, num_samples=None, with_object=False, render=False):
        ...
        if render:
            self.images = []
            # Camera defaults
            self.camera_view_matrix = pybullet.computeViewMatrix(cameraEyePosition=[3., -3., 0.5],
                                                                 cameraTargetPosition=[0., 0., 0.],
                                                                 cameraUpVector=[-1., 0., -1.])
            self.camera_projection_matrix = pybullet.computeProjectionMatrixFOV(fov=60., aspect=1., nearVal=0.1,
                                                                                farVal=3.1)
        ...
        self.shadow_hand_right_urdf = os.path.join("assets", "urdf", "shadow_hand_right_floating.urdf")
    ...
    def on_exit(self, controller):
        ...
        if self.render:
            gif_path = os.path.join("images", "shadow_hand_float.gif")
            imageio.mimsave(gif_path, [np.array(img) for n, img in enumerate(self.images) if n % 2 == 0], fps=10)
            ...
            for hand in frame.hands:
                ...
                if hand.is_right and hand.is_valid and not right_hand_processed:
                    ...
                    if not np.isnan(theta1) and not np.isnan(theta2) and not np.isnan(theta3) and not np.isnan(
                        ...
                        self.shadow_hand_right_target_pos[24+6:29+6] = [
                    ...
                    if not np.isnan(theta1) and not np.isnan(theta2) and not np.isnan(theta3) and not np.isnan(theta4):
                        self.shadow_hand_right_target_pos[3+6:7+6] = [
                    ...
                    if not np.isnan(theta1) and not np.isnan(theta2) and not np.isnan(theta3) and not np.isnan(theta4):
                        self.shadow_hand_right_target_pos[8+6:12+6] = [
                            np.clip(theta1, -0.349, 0.349),
                            np.clip(theta2, 0., 1.571),
                            np.clip(theta3, 0., 1.571),
                            np.clip(theta4, 0., 1.571)]
                    ...
                    if not np.isnan(theta1) and not np.isnan(theta2) and not np.isnan(theta3) and not np.isnan(theta4):
                        self.shadow_hand_right_target_pos[13+6:17+6] = [
                    ...
                    if not np.isnan(theta1) and not np.isnan(theta2) and not np.isnan(theta3) and not np.isnan(theta4):
                        self.shadow_hand_right_target_pos[19+6:23+6] = [
                    ...
                    # WRIST
                    self.shadow_hand_right_target_pos[1+6:3+6] = [
                        -utils.joint_rotation_y(hand.arm.basis, hand.basis, -0.489, 0.140),
                        utils.joint_rotation_x(hand.arm.basis, hand.basis, -0.698, 0.489)]

                    # Armcenter position
                    arm = hand.arm
                    center = arm.elbow_position #+ (arm.wrist_position - arm.elbow_position)
                    wrist_pos = hand.wrist_position
                    diff = wrist_pos - center

                    center = np.array([center[0], -center[2], center[1]]) / 100
                    self.shadow_hand_right_target_pos[1:4] = center.tolist()
                    self.shadow_hand_right_target_pos[4:7] = [diff.roll, diff.pitch, diff.yaw]
